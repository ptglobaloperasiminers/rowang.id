...
export async function POST(req: NextRequest) {
  ...
  const preTranscript = form.get('transcript') as string | null

  // Primary path: browser sends transcript via Web Speech API (free, no extra cost)
  // Fallback: manual text entry from user
  let transcript = preTranscript?.trim() || ''

  if (!transcript) {
    return NextResponse.json({
      error: 'No transcript received. Please use the voice recorder on the page.',
    }, { status: 400 })
  }
  ...
  // Background: Haiku extracts personality patterns (cheap)
  if (transcript.length > 60) {
    extractVoiceInsights(user.id, transcript, rec?.id, duration)
  }

  return NextResponse.json({ id: rec?.id, transcript, duration })
}

// Haiku — background extraction, cheap
