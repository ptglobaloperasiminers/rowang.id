...
type Tab = 'home' | 'interview' | 'voice' | 'memories' | 'chat' | 'clone'
...
const VOICE_PROMPTS = [
  'Talk about the biggest decision of your career — what happened, how you decided, what you learned.',
  ...
  'What does a real, productive day look like for you?',
...
export default function Dashboard() {
  ...
  const sendInterview=async(text?:string)=>{
    ...
    if(ai.includes('Let me ask about')||msgs.length>22){if(!doneCats.includes(iCat!))setDone(p=>[...p,iCat!])}
  ...
  if(status==='loading')return(<div style={{minHeight:'100vh',background:C.cream,display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{color:C.muted}}>Loading...</div></div>)
  ...
  const warmCard:React.CSSProperties={background:C.amberDim,border:`0.5px solid ${C.border}`,borderRadius:'14px',padding:'18px 20px',marginBottom:'12px'}
  ...
  const mlbl:React.CSSProperties={fontFamily:'Inter',fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:'8px'}
  ...
  const userMsg:React.CSSProperties={maxWidth:'84%',padding:'12px 16px',background:C.sand,border:`0.5px solid ${C.border}`,borderRadius:'16px 16px 4px 16px',color:C.walnut,fontSize:'14px',lineHeight:'1.65',alignSelf:'flex-end'}

  const NAV:[Tab,string,string][]=[['home','⌂','Home'],['interview','✦','My Interview'],['voice','◎','Voice'],['memories','○','Memories'],['chat','◐','Test My Clone'],['clone','✧','My Clone']]
    ...
    <div style={{display:'flex',minHeight:'100vh',background:C.cream,fontFamily:'Inter',color:C.espresso}}>
      ...
      <aside style={{position:'fixed',top:0,left:0,height:'100vh',width:'228px',background:'#FDF5E6',borderRight:`0.5px solid ${C.border}`,display:'flex',flexDirection:'column',zIndex:50}}>
        ...
        <div style={{padding:'12px 16px',borderBottom:`0.5px solid ${C.border}`,display:'flex',alignItems:'center',gap:'10px'}}>
          {user?.image?<img src={user.image} style={{width:'30px',height:'30px',borderRadius:'50%',border:`1.5px solid ${C.border}`}} alt=""/>
            :<div style={{width:'30px',height:'30px',borderRadius:'50%',background:C.sandMid,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',fontWeight:500,color:C.walnut}}>{firstName[0]}</div>}
      ...
      <main style={{marginLeft:'228px',flex:1,padding:'36px 40px',maxWidth:'800px'}}>
        ...
        {tab==='home'&&(
          <div className="fade">
            <div style={{...warmCard,padding:'28px'}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:'20px'}}>
                ...
                <div>
                  ...
                  <p style={{fontSize:'14px',color:C.walnut,lineHeight:'1.65',marginBottom:'16px'}}>{conf?.nextAction??"Start your identity interview — it's the highest-impact thing you can do right now."}</p>
                  <div style={{display:'flex',gap:'10px'}}>
                    <button onClick={()=>setTab('interview')} style={btnP}>Start Interview</button>
                    <button onClick={()=>setTab('voice')} style={btnS}>Record Voice</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:'8px',marginTop:'8px'}}>Your progress</div>
            ...
            <div style={{fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:'8px'}}>What would you like to do?</div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
              {([['interview','✦','Identity Interview','A real conversation that builds your AI persona'],['voice','◎','Voice Recording','Speak freely — captured and analyzed'],['memories','○','Add a Memory','Stories, decisions, WhatsApp exports'],['chat','◐','Test My Clone','Talk to your AI — rate every response']] as [Tab,string,string,string][]).map(([id,icon,title,sub])=>(
        ...
        {tab==='interview'&&!iCat&&(
          <div className="fade">
            <div style={{marginBottom:'24px'}}>
              ...
              <p style={{fontSize:'14px',color:C.walnut,lineHeight:'1.65'}}>This is a real conversation — not a form. Each topic has an AI that will ask deep questions, follow up, and keep exploring until it truly understands you. Take your time.</p>
            </div>
                ...
                <div style={{minHeight:'280px',maxHeight:'420px',overflowY:'auto',display:'flex',flexDirection:'column',gap:'12px',padding:'4px 0',marginBottom:'14px'}}>
                  {iMsgs.map((m,i)=>(
                    <div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
                      <div style={m.role==='user'?userMsg:aiMsg}>{!m.content&&iStream&&i===iMsgs.length-1?<Dots/>:m.content}</div>
                    </div>
                  ))}
                  <div ref={iRef}/>
                </div>
        ...
        {tab==='voice'&&(
          <div className="fade">
            <div style={{marginBottom:'24px'}}>
              ...
              <p style={{fontSize:'14px',color:C.walnut,lineHeight:'1.65'}}>Speak naturally about anything. Your voice is your most authentic data source.{vTotal>0?` ${ft(vTotal)} recorded so far.`:''}</p>
            </div>
            ...
            <div style={card}>
              <div style={{fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:'8px'}}>Topics to speak about</div>
              {VOICE_PROMPTS.map((p,i)=>(
                <div key={i} style={{fontSize:'13px',color:C.walnut,padding:'9px 0',borderBottom:i<VOICE_PROMPTS.length-1?`0.5px solid ${C.border}`:'none',lineHeight:'1.55'}}>
        ...
        {tab==='memories'&&(
          <div className="fade">
            ...
            {mTab==='add'&&(
              <div style={card}>
                <div style={{marginBottom:'12px'}}><label style={{fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',display:'block',marginBottom:'6px'}}>Title</label><input value={mTitle} onChange={e=>setMTitle(e.target.value)} placeholder="e.g. How I handled my first major business failure" style={inp}/></div>
                <div style={{marginBottom:'12px'}}>
                  <label style={{fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',display:'block',marginBottom:'6px'}}>Tell the full story</label>
                  ...
                  <textarea rows={8} value={mBody} onChange={e=>setMBody(e.target.value)} placeholder="It was 2019, and we had just received an offer that most people in my position would have taken immediately..." style={ta}/>
                </div>
            ...
            {mTab==='paste'&&(
              <div style={card}>
                ...
                <textarea rows={11} value={mPaste} onChange={e=>setMPaste(e.target.value)} placeholder="Paste WhatsApp export, email threads, or any text that shows how you think..." style={{...ta,fontSize:'13px',marginBottom:'12px'}}/>
                <button onClick={processPaste} disabled={mProc||!mPaste.trim()} style={{...btnP,width:'100%',justifyContent:'center',padding:'12px'}}>{mProc?'Extracting your patterns...':'Extract & Save Patterns →'}</button>
              </div>
            )}
            {mTab==='list'&&(
              <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                {mems.length===0?<div style={{textAlign:'center',padding:'40px',color:C.muted,fontSize:'14px'}}>No memories yet. Add your first one above.</div>
                  :mems.map((m:any)=>(
        ...
        {tab==='chat'&&(
          <div className="fade">
            <div style={{marginBottom:'20px'}}>
              ...
              <p style={{fontSize:'14px',color:C.walnut}}>Talk to your AI. Rate each response — this calibration makes your clone more accurate.</p>
            </div>
            <div style={{...card,padding:0,overflow:'hidden',marginBottom:'12px'}}>
              <div style={{padding:'14px 18px',borderBottom:`0.5px solid ${C.border}`,display:'flex',alignItems:'center',gap:'12px',background:C.amberDim}}>
                {user?.image?<img src={user.image} style={{width:'32px',height:'32px',borderRadius:'50%',border:`1.5px solid ${C.border}`}} alt=""/>
                  :<div style={{width:'32px',height:'32px',borderRadius:'50%',background:C.sandMid,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'13px',fontWeight:500,color:C.walnut}}>{firstName[0]}</div>}
                <div>
              ...
              <div style={{minHeight:'260px',maxHeight:'400px',overflowY:'auto',display:'flex',flexDirection:'column',gap:'12px',padding:'16px'}}>
                {cMsgs.length===0&&<div style={{padding:'24px',textAlign:'center',color:C.muted,fontSize:'14px',lineHeight:'1.65'}}>Ask your clone anything — how it would handle a situation, its opinion on something, a decision you need to make...</div>}
                {cMsgs.map((m,i)=>(
                  <div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
                    <div style={m.role==='user'?userMsg:aiMsg}>{!m.content&&cStream&&i===cMsgs.length-1?<Dots/>:m.content}</div>
                  </div>
                ))}
                <div ref={cRef}/>
              </div>
            ...
            {lastQA&&!cStream&&(
              <div style={card}>
                <div style={{fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:'10px'}}>Was that response accurate to you?</div>
          ...
          <div className="fade">
            ...
            {conf?.isUnlocked?(
              <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                <div style={{...warmCard,textAlign:'center',padding:'32px'}}>
                ...
                <div style={card}>
                  <div style={{fontSize:'11px',color:C.muted,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:'8px'}}>Your clone link</div>
              ...
              <div style={{...warmCard,textAlign:'center',padding:'48px 32px'}}>
...
function Dots(){
  return(<div style={{display:'flex',gap:'4px',padding:'2px 0'}}>{[0,1,2].map(n=><div key={n} style={{width:'6px',height:'6px',borderRadius:'50%',background:'#B47B2E',animation:`bounce 1.4s infinite ${n*.2}s`}}/>)}</div>)
}
